...
public enum Mixins {
    ...
    // Custom world loading screens (loading.properties), gated by the better skies module.
    // 自定义世界加载屏幕（loading.properties），受 better skies 模块总开关控制。
    CUSTOM_LOADING_SCREENS(new Builder("Custom Loading Screens").setSide(Side.CLIENT)
        .setPhase(Phase.EARLY)
        .setApplyIf(
            () -> MCPatcherForgeConfig.instance().betterSkiesEnabled
